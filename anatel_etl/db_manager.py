...
class DBManager:
    ...
    def _get_or_create_id(self, table_name: str, columns_data: dict, query_columns: list) -> int:
        """
        ...
        """
        id_column_name_str = f"id_{table_name.replace('Dim', '').lower()}"
        
        full_table_name = sql.SQL("{}.{}").format(sql.Identifier(self.schema_name), sql.Identifier(table_name))
        ...
        select_query = sql.SQL("SELECT {} FROM {} WHERE {}").format(
            sql.Identifier(id_column_name_str),
            full_table_name,
            select_conditions
        )
        ...
        try:
            ...
            if result:
                ...
                insert_vals_placeholders = sql.SQL(', ').join(sql.Placeholder() * len(columns_data))
            ...
            self.conn.rollback()
            logging.error(f"Erro em _get_or_create_id para {table_name} com dados {columns_data}. Erro: {e}")
            logging.error(f"Query: {self.cursor.query if self.cursor else 'Cursor não disponível'}")
            raise

    def get_or_create_tempo_id(self, ano_mes_str: str) -> int:
        """
        Obtém ou cria um ID para DimTempo.
        Ignora trimestre e nome_mes conforme solicitado.

        Args:
            ano_mes_str (str): String no formato 'YYYY-MM'.

        Returns:
            int: O id_tempo.
        """
        try:
            year, month = map(int, ano_mes_str.split('-'))
            ...
            logging.error(f"Formato de ano_mes inválido: {ano_mes_str}. Erro: {e}")
            raise
    ...
    def bulk_insert_facts(self, fact_data_list):
        ...
        try:
            ...
            self.conn.commit()
            logging.info(f"{len(fact_data_list)} registros inseridos na tabela de fatos.")
        except psycopg2.Error as e:
            self.conn.rollback()
